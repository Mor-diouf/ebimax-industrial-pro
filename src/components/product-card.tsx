...
import { whatsappUrl, type products } from "@/lib/catalog";
  ...
  return (
    <article className="group overflow-hidden border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-product">
      ...
      <div className="p-5">
        ...
        <div className="flex items-end justify-between gap-3"><div><span className="text-[11px] font-bold uppercase text-muted-foreground">Prix</span><p className="font-display text-xl font-black text-foreground">{product.price}</p></div><Button asChild size="icon" aria-label={`Demander un devis pour ${product.title}`}><a href={whatsappUrl(product.title)} target="_blank" rel="noreferrer"><MessageCircle /></a></Button></div>
