    ...
    <>
      <section className="relative min-h-[610px] overflow-hidden bg-foreground text-background lg:min-h-[660px]">
        ...
        <div className="site-container relative flex min-h-[610px] items-center py-16 lg:min-h-[660px]">
          <div className="max-w-2xl">
            <p className="mb-6 flex items-center gap-3 text-xs font-extrabold uppercase tracking-[0.2em] text-primary">
              <span className="h-px w-10 bg-primary" /> Performance sans compromis
            </p>
            <h1 className="font-display text-4xl font-black uppercase leading-[0.95] sm:text-6xl lg:text-7xl">
              Quincaillerie
              <br />
              <span className="text-primary">SOPE S. FALLOU</span>
            </h1>
            <p className="mt-6 max-w-xl text-base leading-7 text-background/75 sm:text-lg">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Button asChild size="lg" className="h-12 px-6 font-extrabold">
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="h-12 border-background/30 bg-background/5 text-background hover:bg-background hover:text-foreground"
              >
                <Link to="/catalog/$slug" params={{ slug: "energie-solaire" }}>
                  Nos Rayons <ArrowRight />
                </Link>
              </Button>
            </div>
          </div>
        </div>
        <div className="absolute bottom-0 left-0 right-0 border-t border-background/15 bg-foreground/75 backdrop-blur">
          <div className="site-container grid grid-cols-2 divide-x divide-background/15 md:grid-cols-4">
            {[
              ...
              return (
                <div
                  key={label as string}
                  className="flex items-center justify-center gap-2 px-2 py-4 text-[11px] font-bold uppercase sm:text-xs"
                >
        ...
        <div className="site-container">
          ...
          <div className="grid auto-rows-[210px] grid-cols-2 gap-3 md:auto-rows-[260px] md:grid-cols-4">
            {storeGallery.map((photo, index) => (
              <figure
                key={photo.src}
                className={`group relative overflow-hidden bg-muted ${photo.featured ? "col-span-2 row-span-2" : ""} ${index === 5 ? "col-span-2" : ""}`}
              >
                <img
                  src={photo.src}
                  alt={photo.alt}
                ...
                <figcaption className="absolute bottom-0 left-0 p-4 font-display text-sm font-extrabold uppercase text-background sm:text-base">
                  {photo.title}
                </figcaption>
              </figure>
            ))}
          ...
          <div className="mt-8 flex flex-col items-start justify-between gap-5 border-l-4 border-primary bg-muted p-6 sm:flex-row sm:items-center">
            ...
            <Button asChild>
              <a href={whatsappUrl()} target="_blank" rel="noreferrer">
                <MessageCircle /> Vérifier la disponibilité
              </a>
            </Button>
          </div>
        </div>
      ...
      <section className="bg-primary py-14">
        <div className="site-container flex flex-col items-start justify-between gap-7 md:flex-row md:items-center">
          ...
          >
            <a href={whatsappUrl()} target="_blank" rel="noreferrer">
              <MessageCircle /> Obtenir un devis
            </a>
          </Button>
        </div>
      </section>
